...
var RotasClass = {
    ...
    botoesRotas: function (rotas) {
        ...
        var possivel = false;
        var validas = [];
        var rotasView = '<div class="col-sm-2 col-xs-4 boxBtnRota"><button title="Limpar Rota" class="btn btn-default resetRota" id="resetRota"> X </button></div>';
        ...
        rotas.forEach(function (rota, index) {
            ...
            rotasView += '<div class="col-sm-2 col-xs-4 boxBtnRota"><button title="Rota ' + (index + 1) + '" id="rota' + index + '" class="btn btn-primary ' + (valida ? 'alert-success' : '') + ' btnRota">' + (index + 1) + '</button></div>';
        });
        ...
        if (possivel) {
            ...
            info += '<div class="col-xs-12 alert alert-success"><b>Destino possível!</b><br><br>';
            info += '<u>Menor possível: <b>' + menorValida.numero + '</b> (' + menorValida.celulas + ' células)</u><br>';
            info += '<u>Maior possível: <b>' + maiorValida.numero + '</b> (' + maiorValida.celulas + ' células)</u></div>';
        } else {
            info += '<div class="col-xs-12 alert alert-danger"><b>Destino impossível!</b><br><br>';
            info += '<u>Menor rota: <b>' + (indexMenorRota + 1) + '</b> (' + menorRota + ' células)</u><br>';
